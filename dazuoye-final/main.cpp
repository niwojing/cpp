...
void gameInit() {
	...
	//加载僵尸死亡图片
	for (int i = 0; i < 10; i++) {
		sprintf_s(name, sizeof(name), "res/ZombieDie/ZombieDie_%d.png", i);
		loadimage(&imgZmDead[i], name);  
	}

	//加载僵尸脑袋掉落
	for (int i = 0; i < 10; i++) {
		sprintf_s(name, sizeof(name), "res/ZombieHead/ZombieHead_%d.png", i);
		loadimage(&imgzmhead[i], name);
	}
...
void updateWindow() {
	...
	for (int i = 0; i < zmMax; i++) {
		if (zms[i].used) {	
			...
			img1 += zms[i].frameIndex;
			if (zms[i].dead) {
				putimagePNG(zms[i].x, zms[i].y - img->getheight(), img);
				putimagePNG(zms[i].x, zms[i].y - img->getheight(), img1);
			}
			else {
				putimagePNG(zms[i].x, zms[i].y - img->getheight(), img);
			}
